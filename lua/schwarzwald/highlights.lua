local c = require('schwarzwald.colors')
local cfg = vim.g.schwarzwald_config
local util = require('schwarzwald.util')
...
function M.setup()
    ...
    if color_name:sub(1, 1) == '$' then
      ...
      if not color_name then
        vim.schedule(function()
          vim.notify(
            'schwarzwald.nvim: unknown color "' .. name .. '"',
            vim.log.levels.ERROR,
            { title = 'schwarzwald.nvim' }
          )
        end)
        return ''
      end
  ...
  for group_name, group_settings in pairs(vim.g.schwarzwald_config.highlights) do
    if group_settings.link then
      vim.cmd(
        string.format('highlight! link %s %s', group_name, group_settings.link)
      )
